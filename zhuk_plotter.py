...
def generate_plots(params):
    ...
    theta_c = params['beta'] - np.pi
    theta_h = 2*np.pi - params['beta']
    ...
    real_coords_x = np.linspace(*params['bounds'][0], sim_N)
    real_coords_y = np.linspace(*params['bounds'][1], sim_N)
    complex_coords = real_coords_x[None,:] + 1j*real_coords_y[:,None]
    circle_coords = np.linspace(0,2*np.pi,sim_N)

    realised_foil_vertices = np.array(complex_points(zhuk(foil(circle_coords)))).T
...
for i in np.linspace(-np.pi/4, np.pi/4, 25):
    params['alpha'] = i
    params['plot_options']['savefile'] = f'tmp/tmp{index}.png'
    fig=generate_plots(params)
    images.append(Image.open(f'tmp/tmp{index}.png'))
    print(index)
    index += 1
